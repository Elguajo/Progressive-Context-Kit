...
class FrameworkContractTests(unittest.TestCase):
    ...
    def test_framework_rule_count(self):
        c=json.loads((ROOT/'docs/contracts/FRAMEWORK_CONTRACT.json').read_text()); self.assertGreaterEqual(c['rule_count'],52); self.assertEqual(c['rule_count'],len(c['rules']))
    ...
    def test_autoresearch_loop_is_single_hypothesis_quality_gated_and_terminal(self):
        c=json.loads((ROOT/'docs/contracts/FRAMEWORK_CONTRACT.json').read_text())
        s=json.loads((ROOT/'docs/evals/static/FRAMEWORK_SCENARIOS.json').read_text())
        rules={r['id']:r for r in c['rules']}
        expected={'FW-050','FW-051','FW-052'}
